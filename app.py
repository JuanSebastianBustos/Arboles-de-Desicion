...
try:
    import AprendizajeRefuerzo
except ImportError as e:
    print(f"Error importando AprendizajeRefuerzo: {e}")
    AprendizajeRefuerzo = None
...
training_status = {"is_training": False, "progress": 0, "total": 0, "current_reward": 0}
...
def rl_practico():
    ...
    try:
        ...
        flash(
            "Error al generar la visualización. Por favor, inténtalo de nuevo.", "error"
        )
    ...
    if request.method == "POST":
        try:
            ...
            valid_transporte, transporte_result = validate_numeric_input(
                ...
                min_val=0,
                max_val=100,
                field_name="Uso de transporte público",
            )
        ...
        try:
            ...
            if experiencia_input not in ["0", "1"]:
                errors["experiencia_previa"] = (
                    "Debe seleccionar si tiene experiencia previa"
                )
...
def dt_practico():
    ...
    try:
        ...
        if request.method == "POST":
            ...
            tipo_inst = int(
                request.form["tipo_institucion"]
            )  # Esto ya es 0 para Publica, 1 para Privada
            ...
            pred_label, pred_prob = DecisionTrees.predict_label(
                promedio, ingresos, personas, tipo_inst
            )

            prediction_result = {"label": pred_label, "probability": f"{pred_prob:.4f}"}
...
# Rutas de Aprendizaje por Refuerzo
@app.route("/refuerzo_practico", methods=["GET", "POST"])
def refuerzo_practico():
    global training_status

    if AprendizajeRefuerzo is None:
        flash("Error: Módulo de Aprendizaje por Refuerzo no disponible.", "error")
        return redirect(url_for("index"))
    ...
    # IMPORTANTE: Solo cargar datos si NO está entrenando
    if not training_status["is_training"]:
        # Intentar obtener estadísticas
        try:
            stats = AprendizajeRefuerzo.get_training_stats()
            if stats and isinstance(stats, dict):
                training_stats = stats
        except Exception as e:
            logger.warning(f"Error obteniendo training_stats: {e}")

        # Intentar obtener gráfico
        try:
            img = AprendizajeRefuerzo.plot_training_progress()
            if img:
                plot_image = img
        except Exception as e:
            logger.warning(f"Error obteniendo plot: {e}")
    ...
    if request.method == "POST":
        ...
        if action == "train":
            if training_status["is_training"]:
                flash(
                    "Ya hay un entrenamiento en progreso. Por favor espera.", "warning"
                )
            else:
                n_episodes = int(request.form.get("n_episodes", 100))

                # Iniciar entrenamiento en thread separado
                thread = threading.Thread(
                    target=train_in_background, args=(n_episodes,)
                )
                thread.daemon = True
                thread.start()

                flash(
                    f"Entrenamiento iniciado con {n_episodes} episodios. Recarga la página en unos minutos para ver resultados.",
                    "info",
                )

                # Redirigir para evitar resubmit
                return redirect(url_for("refuerzo_practico"))

        elif action == "test":
            if training_status["is_training"]:
                flash("Espera a que termine el entrenamiento actual.", "warning")
            else:
                try:
                    logger.info("Iniciando prueba del agente...")
                    results = AprendizajeRefuerzo.test_agent(n_episodes=5)
                    if results and isinstance(results, dict) and "error" not in results:
                        test_results = results
                        flash("Prueba completada exitosamente.", "success")
                    else:
                        flash("Error: No hay modelo entrenado disponible.", "error")
                except Exception as e:
                    logger.error(f"Error en prueba de RL: {e}")
                    flash(f"Error durante la prueba: {str(e)}", "error")

        elif action == "reset":
            if training_status["is_training"]:
                flash(
                    "No puedes reiniciar mientras hay un entrenamiento en progreso.",
                    "warning",
                )
            else:
                try:
                    ...
                    if removed:
                        ...
                        plot_image = None
                        flash("Modelo reiniciado exitosamente.", "success")
                    else:
                        flash("No hay modelo para reiniciar.", "info")
                except Exception as e:
                    logger.error(f"Error al reiniciar modelo RL: {e}")
                    flash(f"Error al reiniciar: {str(e)}", "error")

                # Redirigir después de reset
                return redirect(url_for("refuerzo_practico"))
...
# Función para entrenar en background
def train_in_background(n_episodes):
    global training_status
    try:
        ...
        # Entrenar
        AprendizajeRefuerzo.train_agent(n_episodes=n_episodes)

        training_status["is_training"] = False
        logger.info("Entrenamiento completado en background")
    except Exception as e:
...
def inject_globals():
    ...
    return {"app_name": "ML Supervisado", "app_version": "2.0", "current_year": 2025}
